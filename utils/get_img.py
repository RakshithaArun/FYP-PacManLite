...
def get_image(agent_loc, goal_loc, ghosts_locs=[]):
    ...
    agent_loc = (int((-(agent_loc[1]) * 200) + 205), int((agent_loc[0] * 200) + 205))
    goal_loc = (int((-(goal_loc[1]) * 200) + 205), int((goal_loc[0] * 200) + 205))
    
    for ind, location in enumerate(ghosts_locs):
        ghosts_locs[ind] = [int((-(location[1]) * 200) + 205), int((location[0] * 200) + 205)]
    # read in images used to represent agent and bomb
    image=cv2.imread('utils\\pacman.png')
    ag = cv2.resize(image, dsize=(20, 20))
    image=cv2.imread('utils\\cherry.png')
    gl=cv2.resize(image, dsize=(40, 40))
    image=cv2.imread('utils\\ghost.png')
    gh=cv2.resize(image, dsize=(20, 20))

    # create bg
    img = np.ones((410, 410, 3), dtype=np.uint8)*0
    try:
        ...
        img[a_x-10:a_x+10, a_y-10:a_y+10] = ag
        ...
        for ghost_loc in ghosts_locs:
            ...
            img[gh_x-10:gh_x+10, gh_y-10:gh_y+10] = gh
...
if __name__ == "__main__":
    ...
    plt.imshow(get_image((-0.7, -0.5), (-0.5, 0.5),[[0.5,-0.5],[-0.5,-0.5],[-0.8,0.5],[0.6,0.3]])[0])
    plt.show()
